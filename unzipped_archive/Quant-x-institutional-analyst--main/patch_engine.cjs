...
let text = fs.readFileSync('engine.py', 'utf8');

const init_db_replacement = `def init_db():
...
text = text.replace(/def init_db\(\):[\s\S]*?conn\.close\(\)/, init_db_replacement);

// FIX 1
text = text.replace(/[ \t]*quant_evidence = build_quant_evidence\(\s*win_prob\s*=\s*win_probability if 'win_probability' in dir\(\) else \{\},\s*trade_exp\s*=\s*trade_expectancy if 'trade_expectancy' in dir\(\) else \{\},\s*ml_score\s*=\s*ml_score,\s*backtest\s*=\s*htf_backtest,\s*\)\n/g, '');
text = text.replace(/[ \t]*technical_evidence = build_technical_evidence\(result, htf, etf\)\n/g, '');

const quant_add = `    technical_evidence = build_technical_evidence(result, htf, etf)
    ...
    result['_summary'] = {`;
text = text.replace("    result['_summary'] = {", quant_add);
...
const old_w = `    htf_wyckoff = result.get(htf, {}).get('wyckoff', {}) or {}
...
const new_w = `    avail_set = set(avail)
...
// FIX 14
text = text.replace(`            if len(real_outcomes) >= 20:
                # Train on REAL outcomes from database`, `            win_count_check  = sum(1 for o in real_outcomes if o[1] == 'WIN')
...
// FIX 4
const old_bull = `    BULLISH_KEYWORDS = [
...
const new_bull = `    BULLISH_KEYWORDS = [
        'surge', 'rally', 'jump', 'rise', 'gain', 'high', 'record', 'bull',
        'positive', 'strong', 'growth', 'recovery', 'demand', 'buy', 'upside',
        'breakout', 'support', 'accumulation', 'optimism', 'beat', 'better',
...
text = text.replace(old_bull, new_bull);

text = text.replace(/        'note':             'Raw keyword scoring only\. AI must interpret context and whether sentiment is already priced in\.',/g, "        'note': 'Raw keyword scoring. AI must interpret hawkish/dovish context and whether sentiment is priced in.',\n        'monetary_terms_detected': [kw for kw in MONETARY_KEYWORDS if kw.lower() in ' '.join(i.get('title','') for i in news_items).lower()],");

// FIX 9
text = text.replace(/        candles = candles\[-300:\] if tf in \('1H', '15M'\) else candles\[-500:\]/g, `        SYNTHETIC_ASSETS = {'BOOM1000', 'CRASH1000', 'VOL75', 'VOL100'}
        if asset in SYNTHETIC_ASSETS and tf in ('W1', 'D1'):
            continue
        candles = candles[-300:] if tf in ('1H', '15M') else candles[-500:]`);

fs.writeFileSync('engine.py', text);
console.log('engine.py patched successfully.');
